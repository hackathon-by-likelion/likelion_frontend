...
import "./TogetherPerChoice.css"; // 분리된 CSS 파일을 가져오기

const TogetherPerChoice = ({onSelectPerson}) => {
  ...
  return (
    <div className="together-page">
      <div className="together-wrapper">
        <div className="together-top-info">
          <div>총 {totalPeople}명<br /></div>
          <button className="together_expand-button" onClick={handleToggle}>
            {expanded ? "▲" : "▼"}
          </button>
        </div>
        <div className={expanded ? "together-expanded-content expanded" : "expanded-content"}>
          <div className="together-quantity-control">
            {Object.keys(quantities).map((category) => (
              <div className="together-category-wrapper" key={category}>
                ...
                <div className="together-quantity-buttons">
                  <button
                    className="together-quantity-button"
                    onClick={() => handleDecrement(category)}
                    disabled={!expanded || quantities[category] <= 0}
                  >
                    -
                  </button>
                  <span className="together-quantity-text">{quantities[category]}</span>
                  <button
                    className="together-quantity-button"
        ...
        <button className="together-select" onClick={handleConfirm}>좌석선택</button>
      </div>
    </div>
  );
};

export default TogetherPerChoice;
